...
| Multi-tenancy | All data keyed by `discord_server_id`; a few global leaks flagged (replay index; the `auto_update` *summary* channel `UPDATE_CHANNEL_ID`). **Note:** the per-guild token-cap *ping* channel (`notification_channel_id`) is properly isolated and in active use — it is not a leak. | [brief §3](brief.md#3-multi-tenancy-_closes-gap-3--decision-form-in-adr-004) · [ADR-004](adr-004-multi-tenancy-isolation.md) |
