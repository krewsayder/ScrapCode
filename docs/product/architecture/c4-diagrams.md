...
# Diagrams — `dynamic-tier-registry` (DESIGN wave, target)

> Appended 2026-08-15. **System Context (§1) and Container (§§2, 4) are
> unchanged** — no new external system, no new container, and `bot/tiers.py` is
> a module inside the existing single process. Only a Component diagram is
> warranted. See [ADR-009](adr-009-tier-registry-single-source.md).

## 7. Component diagram — tier resolution path
...
flowchart TB
    ...
    subgraph Write["Write path"]
        Track["bot/tracker.py<br/>get_tier_key → registry<br/>process_api_response<br/>returns IngestReport<br/>(no logging import)"]
    ...
    subgraph Live["Live board lifecycle"]
        Refresh["_refresh_live_leaderboards<br/>+ additive reconciliation<br/>idempotent on tier value<br/>never deletes"]
    end
...
## Notes — `dynamic-tier-registry` diagram

- **Every arrow touching a tier passes through `bot/tiers.py`.** That is the
  whole design, and it is the third instance of the shape ADR-001 drew for
  `bot/permissions.py` and ADR-008 drew for `bot/guild_keys.py`.
- **`bot/tiers.py` has no outward arrows.** No Discord, no storage, no config —
  enforced by import-linter (ADR-009 D8). A rule table with no dependencies is a
  rule table that unit-tests in milliseconds with no event loop and no fixtures.
- **`bot/embeds.py` is marked UNCHANGED and that is a testable claim.** The
  `Tier` value object is structurally compatible with `app_commands.Choice[str]`
  in the two attributes existing code reads, across all 26 raid-tier read sites.
  If a Slice 04 diff touches `embeds.py`, ADR-009 D5 was wrong.
- **The write path reports by return value.** `process_api_response` hands an
  `IngestReport` back to `tasks_cog` rather than logging; `bot/tracker.py` stays
  a pure module. The counters land in the existing `_CycleReport` so one cycle's
  truth stays in one structured record (ADR-009 D7).
- **Reconciliation only ever adds.** A tier that disappears from the registry
  keeps its live message, frozen. Automatic deletion driven by a vendor-shaped
  input is the failure mode the operator's recorded anti-goal rules out.
- **`list_tier_keys` is dashed into Slice 04 alone.** Slices 01–03 touch no
  repository method, which is what keeps Slice 01 at half a day and
