...
class PlayerService:
    ...
    async def refresh_guild(self, discord_server_id: int, guild_id: str, api_key: str) -> None:
        ...
        print(f"[PlayerService] Refreshed {guild_id}: {len(current_ids)} current, {former} former")

    async def validate_if_stale(self, discord_server_id: int, guild_id: str, api_key: str) -> None:
        ...
        if not players:
            await self.refresh_guild(discord_server_id, guild_id, api_key)
            return
